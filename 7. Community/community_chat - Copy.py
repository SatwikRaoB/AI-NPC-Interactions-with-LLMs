...
def get_llm_response(api_type, prompt, context, persona_name, partner_name=None, group_members=None, situation="chat", phase1_informant=None, round_num=0):
    ...
    if situation == "pairwise_sharing":
        ...
        if round_num == 0: # Round 1: State info & find contradiction
            ...
            if persona_name == "Grace":
                 system_prompt += f"2. Compare the story you just paraphrased (Ben blaming Chloe) to what Eva and Finn said earlier (David involved). State the contradiction clearly for the detective."
            elif persona_name == "Harry":
                 system_prompt += f"2. Listen to Grace identify the contradiction involving Ben's story. Analyze this contradiction based on all paraphrased stories and identify Ben's story as the outlier for the detective."
             ...
             system_prompt += f"Based on the contradiction identified in Round 1 (Ben blaming Chloe vs others blaming David), briefly state why Ben's story seems incorrect or suspicious to help {DETECTIVE_NAME} conclude."
        else: system_prompt += f"Briefly comment on the discussion."
        system_prompt += " DO NOT HALLUCINATE or add extra info."
    ...
    # --- Prompt for Detective Ruth's final conclusion ---
        ...
        system_prompt = (
            ...
            f"Their discussion transcript is provided below. Analyze the transcript, specifically focusing on the contradiction identified regarding Ben's statement blaming Chloe versus others stating David was involved. "
